...
function fire1(){
  ...
  // Set canvas drawing surface
  var space = document.getElementById("surface1");
  var surface = space.getContext("2d");
  surface.scale(1,1);
  ...
  var canvasWidth=320;
  var canvasHeight=480;
  $(".wrapper1").css({width:canvasWidth,height:canvasHeight});
  $("#surface1").css({width:canvasWidth,height:canvasHeight});
  ...
  function particle() {
		...
		this.speed = {x: -1+Math.random()*2, y: -5+Math.random()*5};
     canvasWidth = (document.getElementById("surface1").width);
     canvasHeight= (document.getElementById("surface1").height);
     this.location = {x: canvasWidth/2, y: (canvasHeight/2)+35};
